...
$pythonOut = Join-Path $OutputDir ("python\\" + $pythonInstaller)
...
- Python $PythonVersion installer (Windows x64)
- Docling wheelhouse for version $ResolvedDoclingVersion
- Wheelhouse of Python dependencies for Windows x64 / Python $PythonVersion
- Model artifacts in `models\` downloaded by docling-tools

## Install (offline machine)

1) Run the Python installer in python\.
   Suggested options: add Python to PATH and include pip.
2) From this bundle root:
   ~~~powershell
...
- This wheelhouse targets Windows x64 + Python $PythonVersion. Rebuild if your target differs.
